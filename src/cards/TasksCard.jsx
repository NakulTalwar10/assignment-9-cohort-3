...
import { loadTasks, toggleTask, toggleTaskPriority } from "../utils/store";

const TasksCard = ({ darkMode }) => {
  const navigate = useNavigate();
  const [tasks, setTasks] = useState(loadTasks());

  React.useEffect(() => {
    const onFocus = () => setTasks(loadTasks());
          ...
          <div className="flex flex-col h-full">
            ...
            <ul className="space-y-1 flex-1">
              {pending.slice(0, 5).map((task) => (
                <li key={task.id} className="flex items-center gap-1.5 group">
                  <button
                  ...
                  >
                    <svg className="w-3 h-3" fill={task.priority ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
